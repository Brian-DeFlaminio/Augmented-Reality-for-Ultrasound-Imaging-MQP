...
void Caster::newProcessedImage(const void* img, int w, int h, int bpp, int sz)
{
    ...
    //downloadImages(img, w, h, bpp, sz);

    // Load the images to a persistent filepath
    sendToPersistentPath(img, w, h);

    // Load the image into the display
    image_->loadImage(img, w, h, bpp, sz);

    // Send the image to Unity over the network
    //sendImageToUnity(img, w, h, bpp, sz);
}

void Caster::sendToPersistentPath(const void* img, int w, int h) {
    QString baseDirectory = "C:/Users/bdefl/AppData/LocalLow/DefaultCompany/Ultrasound 3D"; //Change

    QString fileName = baseDirectory + "/currentFrame.jpg";
    QImage rawimage(reinterpret_cast<const uchar*>(img), w, h, QImage::Format_RGB32);

    // Check if the image creation was successful
    if (rawimage.isNull()) {
        ui_->status->showMessage("Error creating QImage from image data");
        return;
    }

    // Save the image to the specified file path
    if (rawimage.save(fileName)) {
        //ui_->status->showMessage("Image successfully saved to: " + fileName);
    } else {
        ui_->status->showMessage("Error saving image to: " + fileName);
    }
}



/// Function to download images to a folder on the desktop
/// @param[in] img the image data
/// @param[in] w width of the image
/// @param[in] h height of the image
/// @param[in] bpp the bits per pixel
/// @param[in] sz size of the image in bytes
void Caster::downloadImages(const void* img, int w, int h, int bpp, int sz)
{
    QString baseDirectory = "C:/Users/bdefl/Pictures/.Clarius_Images/";
    ...
    if (!baseDir.exists()) {
        if (!baseDir.mkpath(".")) {
            ui_->status->showMessage("Error creating base directory");
            return;
        }
    }

    // Find the next available folder name (1, 2, 3, etc.)
    int folderIndex = 1;
    QString folderPath = baseDirectory + QString::number(folderIndex);

    // Create the unique folder
    QDir uniqueFolder(folderPath);
    if (!uniqueFolder.mkpath(".")) {
        ui_->status->showMessage("Error creating unique folder");
        return;
    }

    // Save the image within the unique folder with a numerical name starting at 0
    QString fileName = folderPath + "/frame_00" + QString::number(uniqueFolder.count() - 2) + ".jpg";
    QImage image(reinterpret_cast<const uchar*>(img), w, h, QImage::Format_RGB32);

    if (image.save(fileName))
        ui_->status->showMessage("Image successfully saved to " + folderPath);
    else
        ui_->status->showMessage("Error saving image");
}
